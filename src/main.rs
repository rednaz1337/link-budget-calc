        ...
        CentralPanel::default().show(ctx, |ui| {
            ...
            frame_styled(ui).show(ui, |ui| {
                ...
                egui::Grid::new("base_data").num_columns(2).show(ui, |ui| {
                    ui.selectable_value(&mut self.calculation_target, CalculationTarget::Temperature, "Temperature");
                    ui.add(DragValue::new(&mut self.temperature).suffix(" K"));
                    ui.end_row();

                    ui.selectable_value(&mut self.calculation_target, CalculationTarget::Bandwidth, "Bandwidth");
                    ui.add(prefix_drag_value(&mut self.bandwidth).suffix("Hz").range(0.0..=f64::MAX).speed(1e6));
                    ui.end_row();

                    ui.selectable_value(&mut self.calculation_target, CalculationTarget::Snr, "Min. SNR");
                    ui.add(DragValue::new(&mut self.snr_min).suffix(" dB"));
                    ui.end_row();

                    ui.selectable_value(&mut self.calculation_target, CalculationTarget::TxPower, "Tx Power");
                    ui.add(DragValue::new(&mut self.tx_power).suffix(" dBm"));
                    ui.end_row();

                    ui.selectable_value(&mut self.calculation_target, CalculationTarget::RxPower, "Rx Power (sensitivity)");
                    ui.add(DragValue::new(&mut self.rx_power).suffix(" dBm"));
                    ui.end_row();

                    ui.selectable_value(&mut self.calculation_target, CalculationTarget::NoiseFigure, "Noise Figure");
                    ui.add(DragValue::new(&mut self.noise_figure).suffix(" dB"));
                    ui.end_row();

                    ui.selectable_value(&mut self.calculation_target, CalculationTarget::TxGain, "Tx Gain");
                    ui.add(DragValue::new(&mut self.tx_gain).suffix(" dB"));
                    ui.end_row();

                    ui.selectable_value(&mut self.calculation_target, CalculationTarget::RxGain, "Rx Gain");
                    ui.add(DragValue::new(&mut self.rx_gain).suffix(" dB"));
                    ui.end_row();

                    ui.selectable_value(&mut self.calculation_target, CalculationTarget::AdditionalLosses, "AdditionalLosses");
            ...
            frame_styled(&ui).show(ui, |ui| {
                ...
                egui::Grid::new("path_loss").show(ui, |ui| {
                    ui.selectable_value(&mut self.calculation_target, CalculationTarget::Distance, "Distance");
